...
root = Tk()
root.withdraw()
root.fileName = filedialog.askopenfilename(filetypes=(("Lane Video", "*.mp4"), ("All Files", "*.*")))
...
def process_image(img, plot=False):
    ...
    text4 = "Driving Position: Left"
    text5 = "Driving Position: Right"
    text6 = "Driving position: Straight"
    text7 = "Warning: Driving off the lane"
    text8 = "Driving in the lane"
    ...
    if offset < -0.30:
        cv2.putText(result, text4,
                    (60, 160), font, 1.0, (255, 255, 255), thickness=2)
    if offset > 0.30:
        cv2.putText(result, text5,
                    (60, 160), font, 1.0, (255, 255, 255), thickness=2)

    if offset >= 0.6:
        cv2.putText(result, text7,
                    (60, 190), font, 1.0, (255, 19, 3), thickness=2)
    if offset <= -0.6:
        cv2.putText(result, text7,
                    (60, 190), font, 1.0, (255, 19, 3), thickness=2)
...
dist_pickle = 'engine/ldws/wide_dist_pickle.p'
...
video_output = 'output/complex_lane.mp4'
input_path = 'test_videos/project_video2.mp4'

clip1 = VideoFileClip(root.fileName)

final_clip = clip1.fl_image(process_image)
# final_clip.write_videofile(video_output, audio=False)

img = mpimg.imread('engine/ldws/test_images/test_ch4.jpg')
final_clip.preview()
sys.stdout.flush()
